"""Validate YAML division files against the schema and layout rules."""
...
from divisions.paths import expected_path
...
def lint_file(path: Path, data_root: Path) -> list[LintIssue]:
    ...
    # File-location consistency against the OCD id.
    expected = expected_path(div.id, data_root).resolve()
    actual = path.resolve()
    if expected != actual:
        try:
        ...
        issues.append(
            LintIssue(
                path,
                f"file location does not match id {div.id!r}; expected {expected_rel}",
            )
        )

    # Soft checks (warnings-as-issues).
    if not div.sources:
...
def lint_tree(root: Path, data_root: Path | None = None) -> list[LintIssue]:
    ...
    if data_root is None:
        data_root = root if root.is_dir() and root.name == "data" else _find_data_root(root)
    issues: list[LintIssue] = []
